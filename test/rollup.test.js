...
const {getCode, testBundle} = require("./util/test-util");

const typescript = require("../src").default;
...
test("supports overriding tslib with a string", async t => {
	const bundle = await rollup({
		...
		plugins: [typescript({tslib: "export const __extends = (Main, Super) => Main.myParent = Super"})],
		onwarn
	});
...
test("allows specifying a path for tsconfig.json", async t => {
	const bundle = await rollup({
		...
		plugins: [typescript({tsconfig: path.resolve(__dirname, "fixtures/tsconfig-jsx/tsconfig.json")})],
		onwarn
	});
	const code = await getCode(bundle, outputOptions);
...
test("throws if tsconfig cannot be found", async t => {
	...
	t.true(caughtError.message.includes("Could not find specified tsconfig.json"), `Unexpected error message: ${caughtError.message}`);
});
...
test("supports dynamic imports", async t => {
	...
	t.true(code.includes(`console.log("dynamic")`), code);
});
